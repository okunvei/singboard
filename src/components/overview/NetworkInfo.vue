...
function latencyColor(ms: string) {
  const n = Number(ms)
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
      ...
      <div class="bg-base-300/50 rounded-lg p-3 space-y-2 relative">
        ...
        <div class="grid grid-cols-[auto_auto_1fr] gap-x-2 gap-y-1 text-sm">
          <span>微信</span>
          <span>:</span>
          <span :class="latencyColor(latency.wechat)">{{ latency.wechat ? latency.wechat + 'ms' : '未检测' }}</span>
          <span>Bilibili</span>
          <span>:</span>
          <span :class="latencyColor(latency.bilibili)">{{ latency.bilibili ? latency.bilibili + 'ms' : '未检测' }}</span>
          <span>GitHub</span>
          <span>:</span>
          <span :class="latencyColor(latency.github)">{{ latency.github ? latency.github + 'ms' : '未检测' }}</span>
          <span>Cloudflare</span>
          <span>:</span>
          <span :class="latencyColor(latency.cloudflare)">{{ latency.cloudflare ? latency.cloudflare + 'ms' : '未检测' }}</span>
          <span>YouTube</span>
          <span>:</span>
          <span :class="latencyColor(latency.youtube)">{{ latency.youtube ? latency.youtube + 'ms' : '未检测' }}</span>
        </div>
